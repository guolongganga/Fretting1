...
import cn.com.buyforyou.fund.present.fund.InverstUpdatapresent;
...
/**
 * Created by guolonggang on 2018/12/24.
 * Description:确认修改定投
 */

public class InverstUpdataActivity  extends XActivity<InverstUpdatapresent>{
    ...
    private InvestResp investResp1;
    ...
    public int getLayoutId() {
        return R.layout.activity_fund_invest_updata;
    }

    @Override
    public InverstUpdatapresent newP() {
        return new InverstUpdatapresent();
    }
    ...
    public void initEvents() {
        ...
        rlInvestWeek.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                //关闭当前输入框
                KeyBoardUtils.closeKeybord(InverstUpdataActivity.this);
                //显示窗口
...
//        });
    ...
    public void bankCardView(InvestResp resp)
    {
        ...
        //银行图标
        bankLogo = bankCardPageEntity.getBankLogo();
        //银行名称
        bank_name = bankCardPageEntity.getBankName();
        //银行编号
        bankNoTail = bankCardPageEntity.getBankNoTail();
        //每天限额
        limit_per_day = bankCardPageEntity.getLimit_per_day();
        limit_per_payment = bankCardPageEntity.getLimit_per_payment();
        //定投时间和星期几
        exchdate = resp.getExchdate();
        exchWeek = resp.getExchWeek();
        //交易账号
