...
//        }
        ...
        Api.getApi().getNewestFund(reqData)
                .compose(XApi.<NewestFundListResp>getApiTransformer())
                .compose(XApi.<NewestFundListResp>getScheduler())
                .compose(getV().<NewestFundListResp>bindToLifecycle())
                .subscribe(new ApiSubscriber<NewestFundListResp>() {
                    @Override
                    ...
                    @Override
                    public void onNext(NewestFundListResp resp) {
                        if (resp != null && resp.getStatus() == 200) {
                            getV().showData(pageno, resp.getData().getFundMarketInfoQuerys());
                        } else {
